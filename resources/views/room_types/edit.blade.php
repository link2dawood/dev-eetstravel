...
<form method="POST" action="{!! url('room_types') !!}/{!! $room_types->id !!}/update" enctype="multipart/form-data" class="space-y-4">
    ...
    <div class="rounded border border-slate-200 bg-white">
        ...
        <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="name" value="{{ $hasErrors ? '' : $room_types->name }}{{ old('name') }}"
            ...
            <div>
                <label class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{{ trans('main.Code') }}</label>
                <input type="text" name="code" value="{{ $hasErrors ? '' : $room_types->code }}{{ old('code') }}"
                       class="form-control block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600">
            </div>
            <div>
                <label class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{{ trans('main.Sortorder') }}</label>
                <input type="text" name="sort_order" value="{{ $hasErrors ? '' : $room_types->sort_order }}{{ old('sort_order') }}"
                       class="form-control block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600">
            </div>
            <div>
                <label class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{{ trans('Image') }}</label>
                <input type="file" name="image"
                       class="form-control block w-full rounded border border-slate-300 bg-white px-3 py-1.5 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600">
    ...
    <div class="sticky bottom-0 -mx-4 sm:mx-0 sm:static sm:rounded sm:border sm:border-slate-200 bg-white sm:bg-slate-50 px-4 sm:px-5 py-3 border-t border-slate-200 sm:border-t-0 sm:border flex items-center justify-end gap-2 shadow-[0_-4px_8px_-4px_rgba(15,23,42,0.05)] sm:shadow-none">
        <x-ui.button as="a" href="{{ route('room_types.index') }}" variant="secondary">{{ trans('main.Cancel') }}</x-ui.button>
