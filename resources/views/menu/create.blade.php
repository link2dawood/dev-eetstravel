...
<x-ui.page-header
    title="New menu item"
    description="Add a menu item to a tour or service."
    :breadcrumbs="[
...
<form method="POST" action="{{ route('menu.store') }}" class="space-y-4">
    {{ csrf_field() }}
    ...
    <div class="rounded border border-slate-200 bg-white">
        ...
        <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                {{ Form::input('text', 'name', '', ['id' => 'name', 'class' => 'form-control block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600']) }}
            </div>
            <div>
                <label for="price" class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{{ trans('main.Price') }}</label>
                {{ Form::input('text', 'price', '', ['id' => 'price', 'class' => 'form-control block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600']) }}
            </div>
            <div class="md:col-span-2">
                ...
                {{ Form::textarea('description', '', ['class' => 'form-control textarea_editor block w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700', 'id' => 'description']) }}
            </div>
