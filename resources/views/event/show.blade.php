...
<x-ui.page-header
    :title="$event->name"
    description="Event supplier record"
    :breadcrumbs="[
        ['label' => 'Home', 'href' => url('/home')],
        ['label' => 'Events', 'href' => route('event.index')],
        ['label' => $event->name],
    ]"
>
    <x-slot name="actions">
        <x-ui.button as="a" href="{{ route('event.index') }}" variant="ghost" icon="arrow-left">
            {{ trans('main.Back') ?? 'Back' }}
        </x-ui.button>
        @if(Auth::user()->can('event.edit'))
            <x-ui.button as="a" href="{{ route('event.edit', $event->id) }}" variant="secondary" icon="edit">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div class="lg:col-span-2 rounded border border-slate-200 bg-white">
                        <div class="border-b border-slate-200 px-4 py-3 flex items-center gap-2">
                            ...
                            <h2 class="text-sm font-medium text-slate-700">Event information</h2>
                        </div>
                        <dl class="px-4 py-4 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $event->name ?: '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Code</dt>
                                <dd class="mt-0.5 font-mono text-slate-800">{{ $event->code ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Address 1</dt>
                                <dd class="mt-0.5 text-slate-800">{{ $event->address_first ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Address 2</dt>
                                <dd class="mt-0.5 text-slate-800">{{ $event->address_second ?: '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Country</dt>
                                <dd class="mt-0.5 text-slate-800">{{ !empty($event->country) ? (\App\Helper\CitiesHelper::getCountryById($event->country)['name'] ?? '—') : '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">City</dt>
                                <dd class="mt-0.5 text-slate-800">{{ !empty($event->city) ? (\App\Helper\CitiesHelper::getCityById($event->city)['name'] ?? '—') : '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Work phone</dt>
                                <dd class="mt-0.5">
                                    @if($event->work_phone)
                                        <a href="tel:{{ $event->work_phone }}" class="text-primary-700 hover:underline">{{ $event->work_phone }}</a>
                                    @else
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $event->work_fax ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Work email</dt>
                                <dd class="mt-0.5">
                                    @if($event->work_email)
                                        <a href="mailto:{{ $event->work_email }}" class="text-primary-700 hover:underline break-all">{{ $event->work_email }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $event->contact_name ?: '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Contact phone</dt>
                                <dd class="mt-0.5">
                                    @if($event->contact_phone)
                                        <a href="tel:{{ $event->contact_phone }}" class="text-primary-700 hover:underline">{{ $event->contact_phone }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <dd class="mt-0.5">
                                    @if($event->contact_email)
                                        <a href="mailto:{{ $event->contact_email }}" class="text-primary-700 hover:underline break-all">{{ $event->contact_email }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $event->rate_name ?: '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Website</dt>
                                <dd class="mt-0.5">
                                    @if($event->website)
                                        <a href="{{ $event->website }}" target="_blank" class="text-primary-700 hover:underline break-all">{{ $event->website }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <dd class="mt-0.5 text-slate-800 whitespace-pre-line">{{ $event->comments ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Internal comments</dt>
                                <dd class="mt-0.5 text-slate-800 whitespace-pre-line">{{ $event->int_comments ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                    ...
                    <div class="rounded border border-slate-200 bg-white">
                        ...
                        @if($totalCount === 0)
                            <div class="px-4 py-8 text-center">
                                ...
                                <p class="mt-1 text-xs text-slate-500">Attach contracts, schedules, anything related from the Edit page.</p>
                            </div>
                        @else
                            @if($images->count())
                                <div class="px-4 pt-4">
                ...
                <div class="mt-6 rounded border border-slate-200 bg-white">
                    ...
                    <div class="border-t border-slate-200 bg-slate-50 px-4 py-4 rounded-b">
                        <form method="POST" action="{{ route('comment.store') }}" enctype="multipart/form-data" id="form_comment" class="space-y-3">
                            ...
                            <input type="hidden" id="default_reference_id" name="reference_id" value="{{ $event->id }}">
                            <input type="hidden" id="default_reference_type" name="reference_type" value="{{ \App\Comment::$services['event'] ?? 'event' }}">
                            <div class="flex">
