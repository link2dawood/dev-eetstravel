...
<x-ui.page-header
    ...
    :breadcrumbs="[
        ...
        ['label' => $restaurant->name, 'href' => route('restaurant.show', $restaurant->id)],
        ['label' => 'Edit'],
    ]"
>
    <x-slot name="actions">
        <x-ui.button as="a" href="{{ route('restaurant.show', $restaurant->id) }}" variant="ghost" icon="arrow-left">
            {{ trans('main.Back') }}
        </x-ui.button>
    </x-slot>
</x-ui.page-header>
...
<form method="POST" action="{{ route('restaurant.update', ['restaurant' => $restaurant->id]) }}" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    @csrf
    @method('PUT')
    ...
    <div class="lg:col-span-2 space-y-4">
        ...
        <div class="rounded border border-slate-200 bg-white">
            <div class="border-b border-slate-200 px-5 py-3 flex items-start gap-3">
                <div class="flex h-8 w-8 items-center justify-center rounded bg-primary-50 text-primary-600 shrink-0"><x-ui.icon name="utensils" size="sm" /></div>
                <div class="flex-1 min-w-0">
                    <h2 class="text-sm font-medium text-slate-700">Identity</h2>
                    <p class="text-xs text-slate-500">Who is this restaurant?</p>
                </div>
            </div>
            <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="code" name="code" type="text" value="{{ old('code', $restaurant->code) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <input id="address_first" name="address_first" type="text" value="{{ old('address_first', $restaurant->address_first) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <input id="address_second" name="address_second" type="text" value="{{ old('address_second', $restaurant->address_second) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    @if(!empty($restaurant->city) && is_numeric($restaurant->city))
                        @component('component.city_form', [
                            'country_label' => 'country', 'country_translation' => 'main.Country', 'country_default' => $restaurant->country,
                            'city_label'    => 'city',    'city_translation'    => 'main.City',    'city_default'    => \App\Helper\CitiesHelper::getCityById($restaurant->city)['name'] ?? '',
                        ])@endcomponent
                    @else
                        @component('component.city_form', [
                            'country_label' => 'country', 'country_translation' => 'main.Country', 'country_default' => $restaurant->country,
                            'city_label'    => 'city',    'city_translation'    => 'main.City',    'city_default'    => $restaurant->city,
                        ])@endcomponent
                    @endif
                </div>
                <div class="md:col-span-2">
                    ...
                    <input id="website" name="website" type="url" value="{{ old('website', $restaurant->website) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
            </div>
        </div>
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="work_phone" name="work_phone" type="tel" value="{{ old('work_phone', $restaurant->work_phone) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    <label for="work_fax" class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{{ trans('main.WorkFax') }}</label>
                    <input id="work_fax" name="work_fax" type="tel" value="{{ old('work_fax', $restaurant->work_fax) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2">
                    ...
                    <input id="work_email" name="work_email" type="email" value="{{ old('work_email', $restaurant->work_email) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <input id="contact_name" name="contact_name" type="text" value="{{ old('contact_name', $restaurant->contact_name) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <input id="contact_phone" name="contact_phone" type="tel" value="{{ old('contact_phone', $restaurant->contact_phone) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2">
                    ...
                    <input id="contact_email" name="contact_email" type="email" value="{{ old('contact_email', $restaurant->contact_email) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
            </div>
        </div>
                ...
                <div>
                    ...
                    <select id="rate" name="rate"
                        ...
                        @foreach($rates as $rate)
                            <option value="{{ $rate->id }}" {{ old('rate', $restaurant->rate) == $rate->id ? 'selected' : '' }}>{{ $rate->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <div class="px-5 py-5 space-y-4">
                <div>
                    ...
                    <input id="comments" name="comments" type="text" value="{{ old('comments', $restaurant->comments) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <textarea id="int_comments" name="int_comments" rows="3"
                              class="block w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600">{{ old('int_comments', $restaurant->int_comments) }}</textarea>
        ...
        <div class="sticky bottom-0 -mx-4 sm:mx-0 sm:static sm:rounded sm:border sm:border-slate-200 bg-white sm:bg-slate-50 px-4 sm:px-5 py-3 border-t border-slate-200 sm:border-t-0 sm:border flex items-center justify-end gap-2 shadow-[0_-4px_8px_-4px_rgba(15,23,42,0.05)] sm:shadow-none">
            <x-ui.button as="a" href="{{ route('restaurant.show', $restaurant->id) }}" variant="secondary">{{ trans('main.Cancel') }}</x-ui.button>
