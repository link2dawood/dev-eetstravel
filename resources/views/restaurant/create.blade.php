...
<x-ui.page-header
    ...
    description="Add a restaurant supplier to the catalog."
    :breadcrumbs="[
...
<form method="POST" action="{{ route('restaurant.store') }}" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    @csrf
    ...
    <div class="lg:col-span-2 space-y-4">
            ...
            <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name') }}" required
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600 @error('name') border-danger-600 @enderror" />
        ...
        <div class="rounded border border-slate-200 bg-white">
            <div class="border-b border-slate-200 px-5 py-3 flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-500">Reach the restaurant directly.</p>
                </div>
            </div>
            <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
        ...
        <div class="rounded border border-slate-200 bg-white">
            <div class="border-b border-slate-200 px-5 py-3 flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-500">Attach contracts, menus, anything related.</p>
                </div>
            </div>
            <div class="px-5 py-5">
        ...
        <div class="sticky bottom-0 -mx-4 sm:mx-0 sm:static sm:rounded sm:border sm:border-slate-200 bg-white sm:bg-slate-50 px-4 sm:px-5 py-3 border-t border-slate-200 sm:border-t-0 sm:border flex items-center justify-end gap-2 shadow-[0_-4px_8px_-4px_rgba(15,23,42,0.05)] sm:shadow-none">
            <x-ui.button as="a" href="{{ route('restaurant.index') }}" variant="secondary">{{ trans('main.Cancel') }}</x-ui.button>
