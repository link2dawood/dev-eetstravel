        ...
        <div class="box box-primary">
            <div class="box box-body" id="quotation_body" style="border-top: none">
                <div style="margin-bottom: 10px;">
                    ...
                    <button class="btn btn-success roomlist_submit">{!!trans('main.Save')!!}</button>
                </div>
                <script>
                    let tourId = {{$tour->id}};
                    ...
                    <div class="col-md-12">
                        <div class="box box-success">
                            <div class="box-header ui-sortable-handle" style="cursor: move;">
                                ...
                                <h3 class="box-title">{!!trans('main.GuestList')!!}</h3>
                            </div>
                                ...
                                <form id="roomlist_form" action="{{route('guestlist.store')}}" method="POST">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">                                            
                                            ...
                                            <div class="col-md-6">
                                                <div class="input-group">
                                                    <span class="input-group-addon" > {!!trans('main.Hotels')!!}</span>
                                                    <select multiple id="hotelselect" name="hotelIds[]" >
                                                        @foreach($tour->getHotels() as $hotel)
                                                            @if ($hotel->name )<option value="{{ $hotel->id }}">{!! $hotel->name !!}</option> @endif
                                                        @endforeach

                                                    </select>
                                                    <span class="input-group-addon" >
                                                        <input type="checkbox" id="checkboxallhotels">{!!trans('main.SelectAll')!!}
                                                    </span>
                                                </div>
                                            </div>
                                            <div class="col-md-1">
                                                <button type="button" class="btn btn-primary btn-flat pull-right hidden"
                                                        id="roomlist_send">{!!trans('main.SendtoHotels')!!}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <input type="hidden" id="show_url"
                                           value="{{route('roomlist.show', ['id' => $tour->id])}}">
                                    <input type="hidden" id="send_url"
                                           value="{{route('guestlist.send', ['id' => $tour->id])}}">
