...
>
    <x-slot name="actions">
        ...
        @if(Auth::user()->can('rate.edit'))
            <x-ui.button as="a" href="{!! route('rate.edit', $rate->id) !!}" variant="secondary" icon="edit">{{ trans('main.Edit') }}</x-ui.button>
        @endif
    </x-slot>
</x-ui.page-header>
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <dl class="px-5 py-5 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                <div>
                    ...
                    <dd class="mt-0.5 text-slate-800 font-medium">{!! $rate->name !!}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.Mark') }}</dt>
                    <dd class="mt-0.5 text-slate-800">{!! $rate->mark !!}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.Ratetype') }}</dt>
                    <dd class="mt-0.5 text-slate-800">{{ $rate_type == null ? '—' : $rate_type->name }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.Sortorder') }}</dt>
                    <dd class="mt-0.5 text-slate-800">{!! $rate->sort_order !!}</dd>
                </div>
            </dl>
        </div>
    </div>
