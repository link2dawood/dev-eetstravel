...
<x-ui.page-header
    ...
    :breadcrumbs="[
        ...
        ['label' => 'Hotel', 'href' => route('hotel.show', ['hotel' => $hotel->id, 'tab' => 'season_tab'])],
        ['label' => 'Season Price', 'href' => route('hotel.show', ['hotel' => $hotel->id, 'tab' => 'season_tab'])],
        ['label' => 'Edit'],
    ]"
>
    <x-slot name="actions">
...
<form method="POST" action="{{ route('update_season') }}" class="space-y-4">
    {{ csrf_field() }}
        ...
        <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="start_date" id="start_date" value="{{ Carbon\Carbon::parse($agreement->start_date)->format('m-d') }}"
            ...
            <div>
                ...
                <div class="relative">
                    <span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400"><x-ui.icon name="calendar" size="sm" /></span>
                    <input type="text" name="end_date" id="end_date" value="{{ Carbon\Carbon::parse($agreement->end_date)->format('m-d') }}"
                           class="form-control datepickernoyear block w-full h-9 rounded border border-slate-300 bg-white pl-9 pr-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
            </div>
            <div class="md:col-span-2">
            ...
            <div class="md:col-span-2">
                ...
                {!! Form::textarea('description', $agreement->description, ['id' => 'description', 'rows' => 3, 'class' => 'form-control block w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600']) !!}
            </div>
        </div>
    </div>
    ...
    <div class="sticky bottom-0 -mx-4 sm:mx-0 sm:static sm:rounded sm:border sm:border-slate-200 bg-white sm:bg-slate-50 px-4 sm:px-5 py-3 border-t border-slate-200 sm:border-t-0 sm:border flex items-center justify-end gap-2 shadow-[0_-4px_8px_-4px_rgba(15,23,42,0.05)] sm:shadow-none">
        <x-ui.button as="a" href="{{ route('hotel.show', ['hotel' => $hotel->id, 'tab' => 'season_tab']) }}" variant="secondary">{{ trans('main.Cancel') }}</x-ui.button>
