...
<x-ui.page-header
    ...
    :description="$utility_expenses->subject_of_expense ?? null"
    :breadcrumbs="[
...
<form method="POST" action="{{ route('utility_expenses.update', ['utility_expense' => $utility_expenses->id]) }}" enctype="multipart/form-data" class="space-y-4">
    {{ csrf_field() }}
    <input type="hidden" name="office_id" value="1">

    <div class="rounded border border-slate-200 bg-white">
        <div class="border-b border-slate-200 px-5 py-3 flex items-start gap-3">
            <div class="flex h-8 w-8 items-center justify-center rounded bg-primary-50 text-primary-600 shrink-0"><x-ui.icon name="zap" size="sm" /></div>
            <div class="flex-1 min-w-0"><h2 class="text-sm font-medium text-slate-700">Utility expense details</h2></div>
        </div>
        <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
                ...
                <input id="subject_of_expense" name="subject_of_expense" type="text" value="{{ $utility_expenses->subject_of_expense }}"
            ...
            <div>
                ...
                <input class="form-control datepicker block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" name="month" type="text" value="{{ $utility_expenses->month }}" />
            </div>
            <div>
                <label class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{!! trans('Monthly Expense') !!}</label>
                <input id="monthly_expense" name="monthly_expense" type="text" value="{{ $utility_expenses->monthly_expense }}"
                       class="form-control block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
            </div>
        </div>
    </div>
