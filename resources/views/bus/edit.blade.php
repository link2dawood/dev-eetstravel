...
@section('content')
<x-ui.page-header
    :title="'Edit ' . ($bus->name ?: ($bus->license_plate ?: 'Bus #' . $bus->id))"
    description="Bus supplier record"
    :breadcrumbs="[
        ['label' => 'Home', 'href' => url('/home')],
        ['label' => 'Buses', 'href' => route('bus.index')],
        ['label' => $bus->name ?: ($bus->license_plate ?: 'Bus #' . $bus->id), 'href' => route('bus.show', $bus->id)],
        ['label' => 'Edit'],
    ]"
>
    <x-slot name="actions">
        <x-ui.button as="a" href="{{ route('bus.show', $bus->id) }}" variant="ghost" icon="arrow-left">
            {{ trans('main.Back') }}
        </x-ui.button>
    </x-slot>
</x-ui.page-header>
...
<form method="POST" action="{!! url('bus') !!}/{!! $bus->id !!}/update" enctype="multipart/form-data" class="space-y-4">
    ...
    <div class="rounded border border-slate-200 bg-white">
        <div class="border-b border-slate-200 px-5 py-3 flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-slate-500">Name, bus number, and company.</p>
            </div>
        </div>
        <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input id="name" name="name" type="text"
                       value="{{ old('name', ($errors != null && count($errors) > 0) ? '' : $bus->name) }}"
            ...
            <div>
                <label for="bus_number" class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">
                    {{ trans('main.Busnumber') }}
                </label>
                <input id="bus_number" name="bus_number" type="text"
                       value="{{ old('bus_number', ($errors != null && count($errors) > 0) ? '' : $bus->bus_number) }}"
                       class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
            </div>
            <div class="md:col-span-2">
                ...
                {!! Form::select('transfer_id', $transfers,
                    old('transfer_id', ($errors != null && count($errors) > 0) ? '' : $bus->transfer_id),
                    [
                        'id' => 'transfer_id',
                        'class' => 'block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600'
                    ]
    ...
    <div class="sticky bottom-0 -mx-4 sm:mx-0 sm:static sm:rounded sm:border sm:border-slate-200 bg-white sm:bg-slate-50 px-4 sm:px-5 py-3 border-t border-slate-200 sm:border-t-0 sm:border flex items-center justify-end gap-2 shadow-[0_-4px_8px_-4px_rgba(15,23,42,0.05)] sm:shadow-none">
        <x-ui.button as="a" href="{{ route('bus.show', $bus->id) }}" variant="secondary">{{ trans('main.Cancel') }}</x-ui.button>
