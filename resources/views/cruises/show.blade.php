...
>
    <x-slot name="actions">
        ...
        @if(Auth::user()->can('cruises.edit'))
            <x-ui.button as="a" href="{{ route('cruises.edit', $cruise->id) }}" variant="secondary" icon="edit">
...
<div class="rounded border border-slate-200 bg-white">
    ...
    <div class="p-5">
        <div class="tab-content">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div class="lg:col-span-2 rounded border border-slate-200 bg-white">
                        <div class="border-b border-slate-200 px-4 py-3 flex items-center gap-2">
                            <x-ui.icon name="ship" size="sm" class="text-slate-400" />
                            <h2 class="text-sm font-medium text-slate-700">Cruise information</h2>
                        </div>
                        <dl class="px-4 py-4 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $cruise->work_fax ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{!! trans('main.WorkEmail') !!}</dt>
                                <dd class="mt-0.5">
                                    @if($cruise->work_email)
                                        <a href="mailto:{{ $cruise->work_email }}" class="text-primary-700 hover:underline break-all">{{ $cruise->work_email }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $cruise->contact_name ?: '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{!! trans('main.ContactPhone') !!}</dt>
                                <dd class="mt-0.5">
                                    @if($cruise->contact_phone)
                                        <a href="tel:{{ $cruise->contact_phone }}" class="text-primary-700 hover:underline">{{ $cruise->contact_phone }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $cruise->rate_name ?: '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{!! trans('main.Website') !!}</dt>
                                <dd class="mt-0.5">
                                    @if($cruise->website)
                                        <a href="{{ $cruise->website }}" target="_blank" class="text-primary-700 hover:underline break-all">{{ $cruise->website }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <dd class="mt-0.5 text-slate-800 whitespace-pre-line">{{ $cruise->comments ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{!! trans('main.IntComments') !!}</dt>
                                <dd class="mt-0.5 text-slate-800 whitespace-pre-line">{{ $cruise->int_comments ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                    ...
                    <div class="rounded border border-slate-200 bg-white">
                        ...
                        @if($totalCount === 0)
                            <div class="px-4 py-8 text-center">
                                ...
                                <p class="mt-1 text-xs text-slate-500">Attach contracts, route sheets, anything related from the Edit page.</p>
                            </div>
                        @else
                            @if($images->count())
                                <div class="px-4 pt-4">
                ...
                <div class="mt-6 rounded border border-slate-200 bg-white">
                    ...
                    <div class="border-t border-slate-200 bg-slate-50 px-4 py-4 rounded-b">
                        <form method="POST" action="{{ route('comment.store') }}" enctype="multipart/form-data" id="form_comment" class="space-y-3">
                            ...
                            <input type="hidden" id="default_reference_id" name="reference_id" value="{{ $cruise->id }}">
                            <input type="hidden" id="default_reference_type" name="reference_type" value="{{ \App\Comment::$services['cruises'] }}">
                            <div class="flex">
            ...
            {{-- ============================================================ --}}
            {{-- History tab — preserve container ID for JS injection          --}}
            {{-- ============================================================ --}}
...
<span id="services_name" data-service-name="Cruises" data-history-route="{{ route('services_history', ['id' => $cruise->id]) }}" hidden></span>
