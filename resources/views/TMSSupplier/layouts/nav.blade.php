@php
    ...
    $hotel       = $supplier_id ? Hotel::find($supplier_id) : null;
    $hotelName   = $hotel->name ?? 'Supplier';
    $initial     = strtoupper(mb_substr($hotelName, 0, 1));
@endphp
...
<header class="fixed top-0 left-0 right-0 z-30 bg-white border-b border-slate-200 shadow-subtle">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="h-16 flex items-center justify-between gap-4">
            ...
            {{-- Brand --}}
            <a href="{{ route('TMS-Supplier.home') }}" class="flex items-center gap-2 text-slate-900">
                ...
                <span class="text-base font-semibold tracking-tight">TMS Supplier</span>
            </a>

            {{-- Nav links --}}
            <nav class="hidden md:flex items-center gap-1">
                @php $isHome = request()->is('TMS-Supplier/home*'); @endphp
                <a href="{{ route('TMS-Supplier.home') }}"
            ...
            <div class="relative">
                <button type="button" onclick="this.parentElement.classList.toggle('open')"
                    ...
                    <span class="hidden sm:inline text-sm font-medium text-slate-900 max-w-[12rem] truncate">{{ $hotelName }}</span>
