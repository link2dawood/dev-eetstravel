...
@section('content')
<x-ui.page-header
    :title="'Edit ' . $flight->name"
    description="Flight supplier record"
    :breadcrumbs="[
        ['label' => 'Home', 'href' => url('/home')],
        ['label' => 'Flights', 'href' => route('flights.index')],
        ['label' => $flight->name, 'href' => route('flights.show', $flight->id)],
        ['label' => 'Edit'],
    ]"
>
    <x-slot name="actions">
        <x-ui.button as="a" href="{{ route('flights.show', $flight->id) }}" variant="ghost" icon="arrow-left">
            {!! trans('main.Back') !!}
        </x-ui.button>
    </x-slot>
</x-ui.page-header>
...
<span id="place" data-info="{{ $place }}" hidden></span>
...
<form action="{{ route('flights.update', ['flight' => $flight->id]) }}" method="post" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    @csrf
    @method('PUT')
    ...
    <div class="lg:col-span-2 space-y-4">
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ $flight->name }}" required
                ...
                <div>
                    ...
                    <input type="date" name="from_date" id="from_date" value="{{ $flight->from_date }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                ...
                @if($errors->has('date_from'))
                    <div class="md:col-span-2">
                        <p class="text-sm text-danger-700"><strong>{{ $errors->first('date_from') }}</strong></p>
                    </div>
                @endif

                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    @component('component.city_form', [
                        'country_label' => 'country_from', 'country_translation' => 'main.CountryFrom', 'country_default' => $flight->country_from,
                        'city_label'    => 'city_from',    'city_translation'    => 'main.Cityfrom',    'city_default'    => \App\Helper\CitiesHelper::getCityById($flight->city_to)['name'],
                    ])@endcomponent
                </div>
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    @component('component.city_form', [
                        'country_label' => 'country_to', 'country_translation' => 'main.CountryTo', 'country_default' => $flight->country_to,
                ...
                <div class="md:col-span-2">
                    ...
                    <input id="address_first" name="address_first" type="text" value="{{ $flight->address_first }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2">
                    ...
                    <input id="address_second" name="address_second" type="text" value="{{ $flight->address_second }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input id="website" name="website" type="text" value="{{ $flight->website }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
            </div>
        </div>
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="work_phone" name="work_phone" type="text" value="{{ $flight->work_phone }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    <label for="work_fax" class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{!! trans('main.WorkFax') !!}</label>
                    <input id="work_fax" name="work_fax" type="text" value="{{ $flight->work_fax }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2">
                    ...
                    <input id="work_email" name="work_email" type="text" value="{{ $flight->work_email }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <input id="contact_name" name="contact_name" type="text" value="{{ $flight->contact_name }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <input id="contact_phone" name="contact_phone" type="text" value="{{ $flight->contact_phone }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
            </div>
        </div>
                ...
                <div>
                    ...
                    <select name="rate" id="rate"
                        ...
                        @foreach($rates as $rate)
                            <option value="{{ $rate->id }}" {{ ($errors != null && count($errors) > 0 ? old('rate') == $rate->id : $flight->rate == $rate->id) ? 'selected' : '' }}>{{ $rate->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <div class="px-5 py-5 space-y-4">
                <div>
                    ...
                    <input id="comments" name="comments" type="text" value="{{ $flight->comments }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <input id="int_comments" name="int_comments" type="text" value="{{ $flight->int_comments }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
        ...
        <div class="sticky bottom-0 -mx-4 sm:mx-0 sm:static sm:rounded sm:border sm:border-slate-200 bg-white sm:bg-slate-50 px-4 sm:px-5 py-3 border-t border-slate-200 sm:border-t-0 sm:border flex items-center justify-end gap-2 shadow-[0_-4px_8px_-4px_rgba(15,23,42,0.05)] sm:shadow-none">
            <x-ui.button as="a" href="{{ route('flights.show', $flight->id) }}" variant="secondary">{!! trans('main.Cancel') !!}</x-ui.button>
        ...
        <div class="lg:sticky lg:top-4 rounded border border-slate-200 bg-white">
            ...
            <div class="px-5 py-5 space-y-3">
                <div class="flex flex-col gap-2">
                    <x-ui.button type="button" id="btn_generate_map" icon="map-pin" block class="btn_google_maps">
                        @if($place != null)
