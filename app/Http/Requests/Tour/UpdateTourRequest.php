...
class UpdateTourRequest extends FormRequest
{
    ...
    {
        return auth()->check() && auth()->user()->can('tour.edit');
    }
    ...
    public function rules()
    {
        ...
        return [
            'name' => [
                ...
                'max:255',
                'min:3',
                Rule::unique('tours')->ignore($tourId)->whereNull('deleted_at')
            ],
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Check if tour has started (can't modify certain fields)
            $tour = \App\Tour::find($this->route('tour'));
            if ($tour && $tour->departure_date <= now()) {
                if ($this->departure_date !== $tour->departure_date) {
                    $validator->errors()->add('departure_date', 'Cannot modify departure date for tours that have already started.');
                }
                if ($this->pax < $tour->pax) {
                    $validator->errors()->add('pax', 'Cannot reduce passenger count for tours that have already started.');
                }
            }
        });
    }
