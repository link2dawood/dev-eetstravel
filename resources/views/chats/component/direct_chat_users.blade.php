{{-- Loaded into #myModal via .add-direct-chat data-link wiring in main.blade.php
     (specifically /chat/renderUsersForChat). Selectors preserved:
     .chat-user-select[data-user-id] handled in main.blade.php. --}}
            ...
            <ul class="nav nav-stacked list-none m-0 p-0 max-h-[50vh] overflow-y-auto divide-y divide-slate-100">
                @if(count($users))
                    @foreach($users as $user)
                        <li>
                            <a href="#" class="chat-user-select flex items-center gap-2 px-3 py-2.5 text-sm text-slate-700 hover:bg-slate-50"
