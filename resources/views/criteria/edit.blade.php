...
<form method="POST" action="{!! url('criteria') !!}/{!! $criteria->id !!}/update" class="space-y-4">
    ...
    <div class="rounded border border-slate-200 bg-white">
        ...
        <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="name" value="{{ $hasErrors ? '' : $criteria->name }}{{ old('name') }}" required
            ...
            <div>
                <label class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{!! trans('main.ShortName') !!} <span class="text-danger-600">*</span></label>
                <input type="text" name="short_name" value="{{ $hasErrors ? '' : $criteria->short_name }}{{ old('short_name') }}" required
                       class="form-control block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600">
            </div>
            <div>
                <label class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{!! trans('main.Icon') !!}</label>
                <input type="text" name="icon" value="{{ $hasErrors ? '' : $criteria->icon }}{{ old('icon') }}"
                       class="form-control block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600">
            </div>
            <div>
                <label for="criteria_type" class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{!! trans('main.Criteriatype') !!}</label>
                <select name="criteria_type" id="criteria_type"
                        class="form-control block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600">
                    @foreach($criteria_types as $criteria_type)
                        <option value="{{ $criteria_type->id }}" {{ $hasErrors ? (old('criteria_type') == $criteria_type->id ? 'selected' : '') : ($criteria->criteria_type == $criteria_type->id ? 'selected' : '') }}>{{ $criteria_type->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <div class="sticky bottom-0 -mx-4 sm:mx-0 sm:static sm:rounded sm:border sm:border-slate-200 bg-white sm:bg-slate-50 px-4 sm:px-5 py-3 border-t border-slate-200 sm:border-t-0 sm:border flex items-center justify-end gap-2 shadow-[0_-4px_8px_-4px_rgba(15,23,42,0.05)] sm:shadow-none">
        <x-ui.button as="a" href="{{ route('criteria.index') }}" variant="secondary">{{ trans('main.Cancel') }}</x-ui.button>
