...
class UpdateTransferRequest extends FormRequest
{
    ...
    {
        $transferId = $this->route('transfer') ?? $this->route('id');

        return [
            'name' => ValidationService::businessNameRules(true, 191, 2),
            'address_first' => ValidationService::addressRules(true, 500, 5),
            'address_second' => ValidationService::addressRules(false, 500, 5),
            'work_email' => ValidationService::emailRules(false, $transferId ? "transfers,work_email,{$transferId}" : 'transfers,work_email'),
