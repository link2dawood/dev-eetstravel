...
class UpdateRestaurantRequest extends FormRequest
{
    ...
    {
        $restaurantId = $this->route('restaurant') ?? $this->route('id');

        return [
            'name' => array_merge(['required'], ValidationService::businessNameRules(false, 191, 2), [$restaurantId ? "unique:restaurants,name,{$restaurantId}" : 'unique:restaurants,name']),
            'work_email' => ValidationService::emailRules(false, $restaurantId ? "restaurants,work_email,{$restaurantId}" : 'restaurants,work_email'),
