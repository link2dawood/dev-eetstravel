...
>
    <x-slot name="actions">
        ...
        @if(Auth::user()->can('guide.edit'))
            <x-ui.button as="a" href="{{ route('guide.edit', $guide->id) }}" variant="secondary" icon="edit">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div class="lg:col-span-2 rounded border border-slate-200 bg-white">
                        <div class="border-b border-slate-200 px-4 py-3 flex items-center gap-2">
                            ...
                            <h2 class="text-sm font-medium text-slate-700">Guide information</h2>
                        </div>
                        <dl class="px-4 py-4 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $guide->work_fax ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Work email</dt>
                                <dd class="mt-0.5">
                                    @if($guide->work_email)
                                        <a href="mailto:{{ $guide->work_email }}" class="text-primary-700 hover:underline break-all">{{ $guide->work_email }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $guide->contact_name ?: '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Contact phone</dt>
                                <dd class="mt-0.5">
                                    @if($guide->contact_phone)
                                        <a href="tel:{{ $guide->contact_phone }}" class="text-primary-700 hover:underline">{{ $guide->contact_phone }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <dd class="mt-0.5">
                                    @if($guide->contact_email)
                                        <a href="mailto:{{ $guide->contact_email }}" class="text-primary-700 hover:underline break-all">{{ $guide->contact_email }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <dd class="mt-0.5 text-slate-800 whitespace-pre-line">{{ $guide->comments ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Internal comments</dt>
                                <dd class="mt-0.5 text-slate-800 whitespace-pre-line">{{ $guide->int_comments ?: '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Website</dt>
                                <dd class="mt-0.5">
                                    @if($guide->website)
                                        <a href="{{ $guide->website }}" target="_blank" class="text-primary-700 hover:underline break-all">{{ $guide->website }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $guide->rate_name ?: '—' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                ...
                <div class="mt-6 rounded border border-slate-200 bg-white">
                    ...
                    <div class="border-t border-slate-200 bg-slate-50 px-4 py-4 rounded-b">
                        <form method="POST" action="{{ route('comment.store') }}" enctype="multipart/form-data" id="form_comment" class="space-y-3">
                            ...
                            <input type="hidden" id="default_reference_id" name="reference_id" value="{{ $guide->id }}">
                            <input type="hidden" id="default_reference_type" name="reference_type" value="{{ \App\Comment::$services['guide'] ?? 'guide' }}">
                            <div class="flex">
...
<span id="services_name" data-service-name="Guide" data-history-route="{{ route('services_history', ['id' => $guide->id]) }}" hidden></span>
