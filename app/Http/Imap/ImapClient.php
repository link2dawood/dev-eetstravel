...
class ImapClient
{
    ...
    public function getMessagesByCriteriaLatest($criteria = '', $number = 0, $start = 0, $order = 'DESC')
    {
        $emails = array();
        $result = imap_fetch_overview($this->imap, $criteria, SE_UID);
        if(!$result){
            throw new ImapClientException('Messages not found. Or this criteria not supported on your email server.');
        };
        if ($number == 0)
     ...
     */
    public function getMessages($number = 0, $start = 0, $order = 'DESC', $info = true)
    {
        if ($number == 0)
        {
            ...
            if (isset($ids)) {
                foreach ($ids as $id) {
                    if($info){
                        ...
                        $emails[] = $this->getMessageSmallInfo($id);
                    }
                }
            }
        }
    ...
    public function getMessage($id, $decode = IncomingMessage::DECODE)
    {
        ...
        $this->incomingMessage = new IncomingMessage($this->imap, $id, $decode, true);
        return $this->incomingMessage;
    }

    public function getMessageSmallInfo($id, $decode = IncomingMessage::DECODE)
    {
        $this->checkMessageId($id);
    ...
    {
//        dd($ids,$this->imap, implode(",", $ids), $target, CP_UID);
     ...
     * @return bool
     */
    public function setUnseenMessage($ids, $unseen= true)
    {
        // We need better docs for this
        if($unseen)
            return imap_clearflag_full($this->imap, $ids, "\\Seen");

        return imap_setflag_full($this->imap, $ids, "\\Seen \\Flagged");
    }
    /**
     * Add a new folder
     *
    ...
    public function saveMessageInSent($message) {
		//dd( $this->mailbox . $this->getSent());
        return imap_append($this->imap,"{webmail.eetstravel.com:993/imap/ssl}INBOX.Sent", $message, "\\Seen");
    }
