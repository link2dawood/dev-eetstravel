{{-- Tour Tasks table - Monday.com Style --}}
<style>
:root {
...
<div class="monday-wrapper">
    ...
    <div class="monday-group" data-group="todo">
        <div class="monday-group-header" onclick="toggleDashboardGroup('todo')">
            ...
            <span class="monday-group-count">{{ $todoTasks->count() }}</span>
        </div>
                    ...
                    <tr class="monday-table-row" data-task-id="{{ $task->id }}">
                        ...
                        <td>
                            <div class="monday-person">
                                @if($task->show_assigned_users)
                                    @php
                                        $users = explode(' ', trim($task->show_assigned_users));
                                        $displayUsers = array_slice($users, 0, 3);
                                    @endphp
                                    @foreach($displayUsers as $userName)
                                        <div class="monday-avatar" style="background-color: {{ '#' . substr(md5($userName), 0, 6) }}">{{ strtoupper(substr($userName, 0, 2)) }}</div>
                                    @endforeach
                                @endif
                            </div>
                        </td>
                        ...
                        <td>{{ $task->dead_line ? \Carbon\Carbon::parse($task->dead_line)->format('M d') : '-' }}</td>
                        ...
                        <td>
                            ...
                            <span class="monday-epic" style="background-color:#0ea5e920;color:#0ea5e9;">{{ $task->tour_name }}</span>
                            @else
                        ...
                        <td onclick="event.stopPropagation();">
                            <div class="monday-actions">
                                ...
                                @endphp
                                @if($editLink)
                                <a href="{{ $editLink }}" class="monday-action-btn edit" title="Edit">
                                ...
                                @endif
                                @if($deleteLink)
                                <a href="#" onclick="confirmDashboardDelete(event, '{{ $deleteLink }}')" class="monday-action-btn delete" title="Delete">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                        <path d="M4 7l16 0" />
                                        <path d="M10 11l0 6" />
    ...
    <div class="monday-group" data-group="completed">
        <div class="monday-group-header" onclick="toggleDashboardGroup('completed')">
            ...
            <span class="monday-group-count">{{ $completedTasks->count() }}</span>
        </div>
            ...
            <table class="monday-table">
                ...
                <tbody>
                    ...
                    <tr data-task-id="{{ $task->id }}">
                        <td class="monday-table-cell-task">
                            <div class="monday-task-content">
                                <span class="monday-task-text" style="text-decoration:line-through;">{{ $task->content }}</span>
                            </div>
                        </td>
                        <td>
                            <div class="monday-person">
    ...
    <div class="monday-group" data-group="aborted">
        <div class="monday-group-header" onclick="toggleDashboardGroup('aborted')">
            ...
            <span class="monday-group-count">{{ $abortedTasks->count() }}</span>
        </div>
