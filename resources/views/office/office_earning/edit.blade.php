...
@section('content')
<x-ui.page-header
    title="Edit office earning"
    :description="'Month: ' . $office_earnings->month"
    :breadcrumbs="[
...
<form method="POST" action="{{ route('office_earning.update', ['office_earning' => $office_earnings->id]) }}" enctype="multipart/form-data" class="space-y-4">
    {{ csrf_field() }}
    ...
    <div class="rounded border border-slate-200 bg-white">
        ...
        <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input class="form-control datepicker block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" name="month" type="text" value="{{ $office_earnings->month }}" />
            </div>
            <div>
                <label class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{!! trans('Revenue') !!}</label>
                <input id="revenue" name="revenue" type="text" value="{{ $office_earnings->revenue }}"
            ...
            <div>
                ...
                <input id="profit" name="profit" type="text" value="{{ $office_earnings->profit }}"
                       class="form-control block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
            </div>
        </div>
    </div>
