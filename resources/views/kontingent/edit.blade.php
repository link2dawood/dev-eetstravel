...
<x-ui.page-header
    ...
    :breadcrumbs="[
        ...
        ['label' => 'Agreements', 'href' => route('hotel.show', ['hotel' => $hotel->id])],
        ['label' => 'Edit'],
    ]"
>
    <x-slot name="actions">
...
<form method="POST" action="{{ route('update_agreements') }}" class="space-y-4">
    {{ csrf_field() }}
    ...
    <div class="rounded border border-slate-200 bg-white">
        <div class="border-b border-slate-200 px-5 py-3 flex items-start gap-3">
            <div class="flex h-8 w-8 items-center justify-center rounded bg-primary-50 text-primary-600 shrink-0"><x-ui.icon name="file-signature" size="sm" /></div>
        ...
        <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
                ...
                <input type="text" name="name" value="{{ $agreement->name }}"
            ...
            <div>
                ...
                <div class="input-group date relative">
                    ...
                    <input type="text" name="start_date" id="start_date" value="{{ $agreement->start_date }}"
            ...
            <div>
                ...
                <div class="input-group date relative">
                    <span class="input-group-addon absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"><x-ui.icon name="calendar" size="sm" /></span>
                    <input type="text" name="end_date" id="end_date" value="{{ $agreement->end_date }}"
                           class="form-control pull-right datepicker block w-full h-9 rounded border border-slate-300 bg-white pl-8 pr-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600">
                </div>
            </div>

            <div class="md:col-span-2">
                <label class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">Room Types</label>
            ...
            <div class="md:col-span-2">
                ...
                {!! Form::textarea('description', $agreement->description, ['class' => 'form-control block w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600', 'id' => 'description', 'rows' => 4]) !!}
            </div>
        </div>
    </div>
    ...
    <div class="sticky bottom-0 -mx-4 sm:mx-0 sm:static sm:rounded sm:border sm:border-slate-200 bg-white sm:bg-slate-50 px-4 sm:px-5 py-3 border-t border-slate-200 sm:border-t-0 sm:border flex items-center justify-end gap-2 shadow-[0_-4px_8px_-4px_rgba(15,23,42,0.05)] sm:shadow-none">
        <x-ui.button as="a" href="{{ route('hotel.show', ['hotel' => $hotel->id, 'tab' => 'agreement_tab']) }}" variant="secondary">{!! trans('main.Cancel') !!}</x-ui.button>
