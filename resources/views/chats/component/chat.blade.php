...
<div class="rounded border border-slate-200 bg-white shadow-subtle overflow-hidden direct-chat">
    <div class="border-b border-slate-200 px-4 py-3 flex items-start justify-between gap-3">
        ...
        <div class="shrink-0 flex items-center gap-1">
            @if($chat->type == \App\Chat::CHAT_TYPE_CUSTOM)
                <a class="add-direct-chat" id="add_user_to_chat" data-toggle="modal" data-target="#myModal" data-link="/chat/{{ $chat->id }}/renderUsersForCustomChat" title="Add user">
                    <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded text-slate-400 hover:bg-slate-100 hover:text-slate-700">
                        <x-ui.icon name="user-plus" size="xs" />
                    </button>
                </a>
                <a data-toggle="modal" data-target="#myModal" class="delete inline-flex h-7 w-7 items-center justify-center rounded text-danger-500 hover:bg-danger-50 hover:text-danger-600" data-link="{{ route('chat.deleteMsg', ['id' => $chat->id], false) }}" title="Delete chat">
    ...
    <div class="p-4">
        {{-- .direct-chat-messages and the [data-chat-id] are JS hooks. --}}
        <div class="direct-chat-messages min-h-[300px] max-h-[55vh] overflow-y-auto pr-1 space-y-3" data-chat-id="{{ $chat->id }}">
        ...
        <div class="direct-chat-contacts mt-3">
            <ul class="contacts-list list-none m-0 p-0 divide-y divide-slate-100 rounded border border-slate-200">
                @foreach($chat->users as $user)
                    <li>
                        <a href="#" class="flex items-center gap-3 px-3 py-2 hover:bg-slate-50">
                            ...
                            @if($chat->type == \App\Chat::CHAT_TYPE_CUSTOM)
                                <div class="shrink-0">
                                    <a href="#" class="remove-user-from-chat inline-flex h-7 w-7 items-center justify-center rounded text-slate-400 hover:bg-danger-50 hover:text-danger-600"
        ...
        <form action="#" method="post" onsubmit="return false;">
            ...
            <div class="input-group flex items-center gap-2">
                <input type="text"
                       ...
                       data-chat-id="{{ $chat->id }}">
                <button type="submit"
                        class="send-message shrink-0 inline-flex items-center gap-1.5 rounded bg-success-600 px-4 h-9 text-sm font-medium text-white hover:bg-success-700"
                        data-chat-id="{{ $chat->id }}">
