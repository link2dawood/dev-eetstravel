...
@section('title','Restaurant reporting')

@section('content')
<x-ui.page-header
    :title="$restaurant->name"
    description="Restaurant supplier reporting"
    :breadcrumbs="[
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                <div class="rounded border border-slate-200 bg-white">
                    ...
                    <dl class="px-4 py-4 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.Name') }}</dt><dd class="mt-0.5 text-slate-800 font-medium">{!! $restaurant->name !!}</dd></div>
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.Code') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->code !!}</dd></div>
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.AddressFirst') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->address_first !!}</dd></div>
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.AddressSecond') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->address_second !!}</dd></div>
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.Country') }}</dt><dd class="mt-0.5 text-slate-800">{!! !empty($restaurant->country) ? \App\Helper\CitiesHelper::getCountryById($restaurant->country)['name'] : '' !!}</dd></div>
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.City') }}</dt><dd class="mt-0.5 text-slate-800">{!! !empty($restaurant->city) ? \App\Helper\CitiesHelper::getCityById($restaurant->city)['name'] : '' !!}</dd></div>
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.WorkPhone') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->work_phone !!}</dd></div>
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.WorkFax') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->work_fax !!}</dd></div>
                        <div class="sm:col-span-2"><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.WorkEmail') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->work_email !!}</dd></div>
                    </dl>
                </div>
                ...
                <div class="rounded border border-slate-200 bg-white">
                    ...
                    <dl class="px-4 py-4 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.ContactName') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->contact_name !!}</dd></div>
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.ContactPhone') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->contact_phone !!}</dd></div>
                        <div class="sm:col-span-2"><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.ContactEmail') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->contact_email !!}</dd></div>
                        <div class="sm:col-span-2"><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.Comments') }}</dt><dd class="mt-0.5 text-slate-800 prose prose-sm max-w-none">{!! $restaurant->comments !!}</dd></div>
                        <div class="sm:col-span-2"><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.IntComments') }}</dt><dd class="mt-0.5 text-slate-800 prose prose-sm max-w-none">{!! $restaurant->int_comments !!}</dd></div>
                        ...
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.Rate') }}</dt><dd class="mt-0.5 text-slate-800">{!! $restaurant->rate_name !!}</dd></div>
                        <div><dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{{ trans('main.Website') }}</dt><dd class="mt-0.5 text-slate-800 truncate">{!! $restaurant->website !!}</dd></div>
                    </dl>
                </div>
            </div>
