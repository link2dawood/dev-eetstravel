...
<x-ui.page-header
    ...
    :breadcrumbs="[
        ...
        ['label' => $hotel->name, 'href' => route('hotel.show', $hotel->id)],
        ['label' => 'Edit'],
    ]"
>
    <x-slot name="actions">
        <x-ui.button as="a" href="{{ route('hotel.show', $hotel->id) }}" variant="ghost" icon="arrow-left">
            {{ trans('main.Back') }}
        </x-ui.button>
    </x-slot>
</x-ui.page-header>
...
<form method="POST" action="{{ route('hotel.update', ['hotel' => $hotel->id]) }}" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    @csrf
    @method('PUT')
    ...
    <div class="lg:col-span-2 space-y-4">
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name', $hotel->name) }}" required
                ...
                <div>
                    ...
                    <input id="code" name="code" type="text" value="{{ old('code', $hotel->code) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2">
                    <label for="address_first" class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{{ trans('main.AddressFirst') }} <span class="text-danger-600">*</span></label>
                    <input id="address_first" name="address_first" type="text" value="{{ old('address_first', $hotel->address_first) }}" required
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2">
                    ...
                    <input id="address_second" name="address_second" type="text" value="{{ old('address_second', $hotel->address_second) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    @if(!empty($hotel->city) && is_numeric($hotel->city))
                        @component('component.city_form', [
                            'country_label' => 'country', 'country_translation' => 'main.Country', 'country_default' => $hotel->country,
                ...
                <div class="md:col-span-2">
                    ...
                    <input id="website" name="website" type="url" value="{{ old('website', $hotel->website) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
            </div>
        </div>
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <div class="px-5 py-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="work_phone" name="work_phone" type="tel" value="{{ old('work_phone', $hotel->work_phone) }}" required
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    <label for="work_fax" class="block text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">{{ trans('main.WorkFax') }}</label>
                    <input id="work_fax" name="work_fax" type="tel" value="{{ old('work_fax', $hotel->work_fax) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2">
                    ...
                    <input id="work_email" name="work_email" type="email" value="{{ old('work_email', $hotel->work_email) }}" required
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <input id="contact_name" name="contact_name" type="text" value="{{ old('contact_name', $hotel->contact_name) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <input id="contact_phone" name="contact_phone" type="tel" value="{{ old('contact_phone', $hotel->contact_phone) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div class="md:col-span-2">
                    ...
                    <input id="contact_email" name="contact_email" type="email" value="{{ old('contact_email', $hotel->contact_email) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
            </div>
        </div>
                ...
                <div>
                    ...
                    <select id="rate" name="rate"
                        ...
                        @foreach($rates as $rate)
                            <option value="{{ $rate->id }}" {{ old('rate', $hotel->rate) == $rate->id ? 'selected' : '' }}>{{ $rate->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <div class="px-5 py-5 space-y-4">
                <div>
                    ...
                    <input id="comments" name="comments" type="text" value="{{ old('comments', $hotel->comments) }}"
                           class="block w-full h-9 rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600" />
                </div>
                <div>
                    ...
                    <textarea id="int_comments" name="int_comments" rows="3"
                              class="block w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 shadow-subtle focus:outline-none focus:ring-2 focus:ring-primary-600/30 focus:border-primary-600">{{ old('int_comments', $hotel->int_comments) }}</textarea>
        ...
        <div class="sticky bottom-0 -mx-4 sm:mx-0 sm:static sm:rounded sm:border sm:border-slate-200 bg-white sm:bg-slate-50 px-4 sm:px-5 py-3 border-t border-slate-200 sm:border-t-0 sm:border flex items-center justify-end gap-2 shadow-[0_-4px_8px_-4px_rgba(15,23,42,0.05)] sm:shadow-none">
            <x-ui.button as="a" href="{{ route('hotel.show', $hotel->id) }}" variant="secondary">Cancel</x-ui.button>
...
<script>
    // Inline file delete handler — same pattern as /clients/{id}/edit.
