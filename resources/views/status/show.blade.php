...
>
    <x-slot name="actions">
        ...
        @if(Auth::user()->can('status.edit'))
            <x-ui.button as="a" href="{!! route('status.edit', $status->id) !!}" variant="secondary" icon="edit">{{ trans('main.Edit') }}</x-ui.button>
        @endif
    </x-slot>
</x-ui.page-header>
        ...
        <div class="rounded border border-slate-200 bg-white">
            ...
            <dl class="px-5 py-5 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                <div>
                    ...
                    <dd class="mt-0.5 text-slate-800 font-medium">{!! $status->name !!}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{!! trans('main.Type') !!}</dt>
                    <dd class="mt-0.5 text-slate-800">{!! !empty($status_type->name) ? $status_type->name : '—' !!}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{!! trans('main.Sortorder') !!}</dt>
                    <dd class="mt-0.5 text-slate-800">{!! $status->sort_order !!}</dd>
                </div>
                @if(!empty($status->color))
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">{!! trans('main.Color') !!}</dt>
                        <dd class="mt-0.5 inline-flex items-center gap-2">
                            <span class="inline-block h-4 w-4 rounded border border-slate-200" style="background-color: {{ $status->color }}"></span>
                            <span class="font-mono text-slate-700">{{ $status->color }}</span>
                        </dd>
                    </div>
                @endif
            </dl>
