...
>
    <x-slot name="actions">
        ...
        @if(Auth::user()->can('driver.edit'))
            <x-ui.button as="a" href="{{ route('driver.edit', $driver->id) }}" variant="secondary" icon="edit">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div class="lg:col-span-2 rounded border border-slate-200 bg-white">
                        <div class="border-b border-slate-200 px-4 py-3 flex items-center gap-2">
                            ...
                            <h2 class="text-sm font-medium text-slate-700">Driver information</h2>
                        </div>
                        <dl class="px-4 py-4 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                            <div>
                                ...
                                <dd class="mt-0.5 text-slate-800">{{ $driver->name ?: '—' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Phone</dt>
                                <dd class="mt-0.5">
                                    @if($driver->phone)
                                        <a href="tel:{{ $driver->phone }}" class="text-primary-700 hover:underline">{{ $driver->phone }}</a>
                                    @else
                            ...
                            <div>
                                ...
                                <dd class="mt-0.5">
                                    @if($driver->email)
                                        <a href="mailto:{{ $driver->email }}" class="text-primary-700 hover:underline break-all">{{ $driver->email }}</a>
                                    @else
                                        <span class="text-slate-400">—</span>
                                    @endif
                                </dd>
                            ...
                            <div>
                                <dt class="text-xs font-medium uppercase tracking-wide text-slate-500">Bus Company</dt>
                                <dd class="mt-0.5 text-slate-800">{{ @$driver->transfer->name ?: '—' }}</dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <div class="rounded border border-slate-200 bg-white">
                        ...
                        @if($totalCount === 0)
                            <div class="px-4 py-8 text-center">
                                ...
                                <p class="mt-1 text-xs text-slate-500">Attach documents, anything related from the Edit page.</p>
                            </div>
                        @else
                            @if($images->count())
                                <div class="px-4 pt-4">
                ...
                <div class="mt-6 rounded border border-slate-200 bg-white">
                    ...
                    <div class="border-t border-slate-200 bg-slate-50 px-4 py-4 rounded-b">
                        <form method="POST" action="{{ route('comment.store') }}" enctype="multipart/form-data" id="form_comment" class="space-y-3">
                            ...
                            <input type="hidden" id="default_reference_id" name="reference_id" value="{{ $driver->id }}">
                            <input type="hidden" id="default_reference_type" name="reference_type" value="{{ \App\Comment::$services['driver'] ?? 'driver' }}">
                            <div class="flex">
